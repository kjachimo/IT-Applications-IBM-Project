...
def log_result(speed, avg_track_pos):
    with open("experiment_results.txt", "a") as f:
        f.write(f"{time.time()}, speed={speed}, avg_track_pos={avg_track_pos}\n")
...
if __name__ == "__main__":
    ...
    if EXPERIMENT_MODE:
        test_speeds = [50, 100, 150]
        
        for speed in test_speeds:
            print(f"\nRunning experiment for TARGET_SPEED={speed}")
            ...
            for step in range(500):  # krótki test
                C.get_servers_input()
                S = C.S.d

                track_positions.append(abs(S['trackPos']))

                drive_modular(C)
                C.respond_to_server()

            avg_track_pos = sum(track_positions) / len(track_positions)

            print(f"Result: avg_track_pos={avg_track_pos}")
            log_result(speed, avg_track_pos)
